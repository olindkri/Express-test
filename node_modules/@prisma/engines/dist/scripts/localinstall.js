...
var require_semver = __commonJS({
  "../../node_modules/.pnpm/semver@6.3.1/node_modules/semver/semver.js"(exports, module2) {
    ...
    src[t3.NUMERICIDENTIFIERLOOSE] = "\\d+";
    tok("NONNUMERICIDENTIFIER");
    src[t3.NONNUMERICIDENTIFIER] = "\\d*[a-zA-Z-]" + LETTERDASHNUMBER + "*";
    tok("MAINVERSION");
    ...
    tok("BUILDIDENTIFIER");
    src[t3.BUILDIDENTIFIER] = LETTERDASHNUMBER + "+";
    tok("BUILD");
    ...
    safeRe[t3.COERCERTL] = new RegExp(makeSafeRe(src[t3.COERCE]), "g");
    tok("LONETILDE");
    src[t3.LONETILDE] = "(?:~>?)";
    tok("TILDETRIM");
    src[t3.TILDETRIM] = "(\\s*)" + src[t3.LONETILDE] + "\\s+";
    re[t3.TILDETRIM] = new RegExp(src[t3.TILDETRIM], "g");
    safeRe[t3.TILDETRIM] = new RegExp(makeSafeRe(src[t3.TILDETRIM]), "g");
    ...
    safeRe[t3.CARETTRIM] = new RegExp(makeSafeRe(src[t3.CARETTRIM]), "g");
    ...
    safeRe[t3.COMPARATORTRIM] = new RegExp(makeSafeRe(src[t3.COMPARATORTRIM]), "g");
    ...
    Comparator.prototype.parse = function(comp) {
      var r2 = this.options.loose ? safeRe[t3.COMPARATORLOOSE] : safeRe[t3.COMPARATOR];
      var m = comp.match(r2);
      if (!m) {
    ...
    Range.prototype.parseRange = function(range) {
      ...
      var hr = loose ? safeRe[t3.HYPHENRANGELOOSE] : safeRe[t3.HYPHENRANGE];
      range = range.replace(hr, hyphenReplace);
      debug4("hyphen replace", range);
      range = range.replace(safeRe[t3.COMPARATORTRIM], comparatorTrimReplace);
      debug4("comparator trim", range, safeRe[t3.COMPARATORTRIM]);
      range = range.replace(safeRe[t3.TILDETRIM], tildeTrimReplace);
      range = range.replace(safeRe[t3.CARETTRIM], caretTrimReplace);
      range = range.split(/\s+/).join(" ");
      var compRe = loose ? safeRe[t3.COMPARATORLOOSE] : safeRe[t3.COMPARATOR];
    ...
    function replaceTilde(comp, options) {
      var r2 = options.loose ? safeRe[t3.TILDELOOSE] : safeRe[t3.TILDE];
    ...
    function replaceCaret(comp, options) {
      ...
      var r2 = options.loose ? safeRe[t3.CARETLOOSE] : safeRe[t3.CARET];
      return comp.replace(r2, function(_, M, m, p2, pr) {
    ...
    function replaceXRange(comp, options) {
      comp = comp.trim();
      var r2 = options.loose ? safeRe[t3.XRANGELOOSE] : safeRe[t3.XRANGE];
    ...
    function replaceStars(comp, options) {
      ...
      return comp.trim().replace(safeRe[t3.STAR], "");
    }
    ...
    function coerce(version, options) {
      ...
      var match = null;
      if (!options.rtl) {
        match = version.match(safeRe[t3.COERCE]);
      } else {
        var next;
        while ((next = safeRe[t3.COERCERTL].exec(version)) && (!match || match.index + match[0].length !== version.length)) {
...
async function main() {
  ...
  const engineCachePaths = {
    ...
    ["schema-engine" /* SchemaEngineBinary */]: import_path2.default.join(cacheDir, "schema-engine" /* SchemaEngineBinary */)
  };
  if (branch !== void 0) {
  ...
  if (folder !== void 0) {
    ...
    const engineOutputPaths = {
      ...
      ["schema-engine" /* SchemaEngineBinary */]: import_path2.default.join(folder, "schema-engine" /* SchemaEngineBinary */.concat(binExt))
    };
